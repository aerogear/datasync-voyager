...
Your mobile app can run offline and allow users to query and create mutations using the offix-client-boost module. 
...
The `offix-client-boost` module also provides cache helper functions to reduce the amount of code required, as described in xref:cache-update-helpers[].
...
The `offix-client-boost` module provides an `OfflineClient` class which exposes the following functionality:
...
The offix-client-boost module provides an `offlineMutate` method which extends Apollo's mutate function with some extra functionality. 
...
[source,javascript]
----
  const { CacheOperation } = require('offix-client-boost');
...
[source, javascript]
----
const { createMutationOptions, CacheOperation } = require('offix-client-boost');
...
The offix-client-boost module provides an out of the box solution for managing updates to your application's cache.
...
The offix-client-boost module provides a subscription helper which can generate the necessary options to be used with Apollo Client's `subscribeToMore` function.
...
[source, javascript]
----
const { createSubscriptionOptions } = require('offix-client-boost');
...
The offix-client-boost module provides the ability to automatically call `subscribeToMore` on your `ObservableQuery`. 
...
To avoid this, use the `subscribeToMoreHelper` function from the offix-client-boost module to automatically handle this by passing an array of subscriptions and their corresponding queries:
...
import { CordovaNetworkStatus, NetworkInfo } from 'offix-client-boost';
