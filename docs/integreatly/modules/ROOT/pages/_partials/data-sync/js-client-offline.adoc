...
Your mobile app can run offline and allow users to query and create mutations using the @aerogear/voyager-server module. 
...
DataSync offers `offlineMutate` method additionally to `mutate` that will enable 
performing mutation operations when offline. 
...
The `@aerogear/voyager-server` module also provides cache helper functions to reduce the amount of code required, as described in xref:cache-update-helpers[].
...
The `@aerogear/voyager-server` module provides an `OfflineClient` class which exposes the following functionality:
...
The @aerogear/voyager-server module provides an `offlineMutate` method which extends Apollo's mutate function with some extra functionality. 
...
[source,javascript]
----
  const { CacheOperation } = require('@aerogear/voyager-server');
...
[source, javascript]
----
const { createMutationOptions, CacheOperation } = require('@aerogear/voyager-server');
...
The @aerogear/voyager-server module provides an out of the box solution for managing updates to your application's cache.
...
The @aerogear/voyager-server module provides a subscription helper which can generate the necessary options to be used with Apollo Client's `subscribeToMore` function.
...
[source, javascript]
----
const { createSubscriptionOptions } = require('@aerogear/voyager-server');
...
The @aerogear/voyager-server module provides the ability to automatically call `subscribeToMore` on your `ObservableQuery`. 
...
To avoid this, use the `subscribeToMoreHelper` function from the @aerogear/voyager-server module to automatically handle this by passing an array of subscriptions and their corresponding queries:
...
import { CordovaNetworkStatus, NetworkInfo } from '@aerogear/voyager-server';
