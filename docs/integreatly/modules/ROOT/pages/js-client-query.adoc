[id='querying-a-data-sync-server-using-a-data-sync-client']
...
Data Sync provides JavaScript libraries which integrate your javascript app  with a server that also uses Data Sync.  
The client libraries are based on the link:https://www.apollographql.com/docs/react/[Apollo client].
...
* You have created an empty web project that supports ES6, using the link:https://webpack.js.org/guides/getting-started/[webpack getting started] guide.
...
----
+
NOTE: A prerequisite is that you have created an empty web project that supports ES6, using the link:https://webpack.js.org/guides/getting-started/[webpack getting started] guide.
...
If the client app uses subscriptions, then the `wsUrl` field is also required.
+
.src/index.js
[source,javascript]
----
